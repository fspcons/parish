...
help:
	...
	@echo "  make infra-start    - Start local infrastructure (Firestore emulator + Redis)"
	@echo "  make infra-stop     - Stop local infrastructure"
	@echo "  make run-local      - Start infrastructure + run the API locally"
	@echo "  make smoke-test     - Run happy-path smoke tests (optional: SMOKE_CLEAR_EMULATOR=1 wipes emulator on exit; restart API after)"
...
infra-start:
	@echo "Starting local infrastructure (Firestore emulator + Redis)..."
	docker compose up -d
	...
	@echo "Infrastructure ready (Firestore :8081, Redis :6379)"
